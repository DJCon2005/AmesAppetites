...
const Homepage = () => {
    ...
    const fetchHomepageData = async () => {
      try {
        ...
        const res = await fetch(
          "https://ames-appetites-backend.vercel.app/api/restaurants",
        );
        if (res.ok) {
        ...
        if (token) {
          const favRes = await fetch(
            "https://ames-appetites-backend.vercel.app/api/favorites",
            {
  ...
  return (
    <main className="max-w-6xl mx-auto px-6 py-8">
      {/* Header / Title */}
      <div className="mb-10">
        <h1 className="text-white drop-shadow-md text-center text-4xl font-bold mb-2">
          Welcome to
        </h1>
        <h2 className="text-red-800 drop-shadow-md text-center text-6xl font-extrabold tracking-tight">
          Ames Appetites
        </h2>
        <br />
        ...
        <p class="text-black drop-shadow-md text-center text-sm font-semibold">
          You can use the search bar at the top to search for a specific
          restaurant in the Ames area, you then click on the restaurant to access the details page. If you wish to search by price or
          cuisine, you can click on All Restaurants and use the drop down menues
            ...
            >
              ...
              <div className="p-4 flex flex-col grow">
                ...
                <div className="mt-auto pt-3 inline-flex [&_svg]:inline-block">
                  <Rating
                    initialValue={
                      Math.round((restaurant.averageRating || 0) * 2) / 2
                    }
                    readonly={true}
                    size={20}
                    allowFraction={true}
                  />
                  <span className="text-xs text-stone-500 ml-2 mt-1">
      ...
      <div className="mb-8 bg-white/65 backdrop-blur-md rounded-xl shadow-md border border-white/20 p-6 flex flex-col relative h-auto min-h-[14rem]">
        ...
        {!localStorage.getItem("token") ? (
          <div className="flex-1 flex flex-col items-center justify-center">
            ...
            <Link
              to="/login"
              className="bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-6 rounded-xl shadow-sm transition-colors"
            >
              Log In
            </Link>
          </div>
          ...
          <div className="flex overflow-x-auto gap-4 pb-4 snap-x">
            {favorites.map((fav) => (
              ...
              >
                ...
                />
                <div className="p-3">
                  <h3 className="font-bold text-red-700 truncate">
                    {fav.title}
                  </h3>
                  <p className="text-xs text-stone-600 truncate">
                    {fav.categories?.join(", ") || "N/A"}
                  </p>
                </div>
              </Link>
            ))}
